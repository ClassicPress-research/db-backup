...
abstract class Driver implements DatabaseInterface
{
	...
	public static function getConnectors($tech = null)
	{
		...
		foreach ($iterator as $file)
		{
			...
			$class = str_ireplace('.php', '', '\\ClassicPress\\SimpleDBBackup\\Database\\Driver\\' . ucfirst(trim($fileName)));
	...
	public static function getInstance($options = array())
	{
		...
		if (empty(self::$instances[$signature]))
		{
			...
			$class = '\\ClassicPress\\SimpleDBBackup\\Database\\Driver\\' . ucfirst($options['driver']);
	...
	public function getQuery($new = false)
	{
		...
		{
			// Derive the class name from the driver.
			$class = '\\ClassicPress\\SimpleDBBackup\\Database\\Query\\' . ucfirst($this->name);
	...
	public function getColumnsMeta($tableName)
	{
		...
		if (empty($result))
		{
			throw new RuntimeException(sprintf("Table %s does not exist in database %s or the current database user does not have permissions to retrieve its column metadata", $tableName, $database));
		}
